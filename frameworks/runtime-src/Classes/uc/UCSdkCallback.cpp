...
//		CUCGameSdk::notifyZone("66区-风起云涌", "R29924", "Role-大漠孤烟");
        ...
        char cmd[1000];
        sprintf(cmd, "ucsdk log_succ %s", UCSdk::s_sid.c_str());
        esocket_receive(cmd);

	} else if (code == CUCStatusCode::LOGIN_EXIT) {
		//登录界面退出，返回到游戏画面
		CCLog("login UI exit, back to game UI");
        esocket_receive("ucsdk log_cancel");
	} else {
		CCLog("Failed login, code=%d, msg=%s", code, pszmsg);
        char cmd[1000];
        sprintf(cmd, "ucsdk log_fail %d %s", code, pszmsg);
        esocket_receive(cmd);
//		CCMessageBox(pszmsg,"回调提示" );
		//登录失败
		//
...
void UCSdkCallback::logout_callback(int code, const char* pszmsg) {
	...
	if (code == CUCStatusCode::SUCCESS) {
		...
		UCSdk::s_sid = "";
//		CCMessageBox(pszmsg,"回调提示" );
        esocket_receive("ucsdk logout_succ");
	} else {
		//unknown error
		CCLog("unknown error: code=%d, msg=%s", code, pszmsg);
//		CCMessageBox(pszmsg,"回调提示" );
        char cmd[1000];
        sprintf(cmd, "ucsdk logout_fail %d %s", code, pszmsg);
        esocket_receive(cmd);
	}
}
...
//		CCMessageBox("user closed the SDK UI, backed to game UI","回调提示" );
        esocket_receive("ucsdk sdkclose");
	}
}
...
//	CCMessageBox("received order_callback","回调提示" );
	if (code == CUCStatusCode::SUCCESS) {
		...
		CCLog("received order_callback order info: code=%d, orderId=%s, orderAmount=%.2f, payWayId=%d, payWayName=%s", code, pszOrderId, orderAmount, payWayId, pszPayWayName);
        char cmd[1000];
        sprintf(cmd, "ucsdk pay_succ %d %s %.2f %d %s", code, pszOrderId, orderAmount, payWayId, pszPayWayName);
        esocket_receive(cmd);
	} else if (code == CUCStatusCode::PAY_USER_EXIT) {
		//充值界面已关闭，回到游戏画面，游戏应根据实际需要决定是否进行画面刷新
        esocket_receive("ucsdk pay_close");
	} else {
		//充值调用失败
        esocket_receive("ucsdk pay_fail");
	}
...
void UCSdkCallback::userCenter_callback(int code, const char* pszmsg) {
	...
	if (code == CUCStatusCode::SUCCESS) {
        ...
        esocket_receive("ucsdk center_out_succ");
	} else {
		//fail
		CCLog("unknown error: code=%d, msg=%s", code, pszmsg);
        char cmd[1000];
        sprintf(cmd, "ucsdk center_out_fail %d %s", code, pszmsg);
        esocket_receive(cmd);
	}
}
...
void UCSdkCallback::uPointCharge_callback(int code, const char* pszmsg) {
	...
	if (code == CUCStatusCode::SDK_CLOSE) {
		...
		CCLog("user closed the user center UI, backed to game UI");
        esocket_receive("ucsdk ucharge_succ");
	} else if (code == CUCStatusCode::NO_INIT) {
		CCLog("not inited");
        esocket_receive("ucsdk ucharge_notinited");
	} else if (code == CUCStatusCode::NO_LOGIN) {
		CCLog("not logined");
        esocket_receive("ucsdk ucharge_notlogined");
	} else {
		//unknown error
		CCLog("unknown error: code=%d, msg=%s", code, pszmsg);
        char cmd[1000];
        sprintf(cmd, "ucsdk ucharge_fail %d %s", code, pszmsg);
        esocket_receive(cmd);
	}
}
...
void UCSdkCallback::enterUI_callback(int code, const char* pszmsg) {
    ...
    char cmd[1000];
    sprintf(cmd, "ucsdk enterui %d %s", code, pszmsg);
    esocket_receive(cmd);
}

void UCSdkCallback::exit_callback(int code, const char* pszmsg) {
	CCLog("received exit_callback: code=%d, msg=%s", code, pszmsg);
	if(code==-703)
	{//继续游戏
	}else if(code == -702)
	{//退出游戏
