...
public:
	...
	static int gameUserLogin_callback(const char *pszUsername,
			const char *pszPassword, char **lpSid);

	static void logout_callback(int code, const char *pszmsg);
	static void floatMenu_callback(int code, const char *pszmsg);

	static void pay_callback(int code, const char *pszOrderId,
	...
	static void exit_callback(int code, const char *pszmsg);
};
