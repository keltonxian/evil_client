...
function main()
	...
	if true == RES_DEBUG then
		local platform = cc.Application:getInstance():getTargetPlatform();
		if platform == cc.PLATFORM_OS_IPHONE or platform == cc.PLATFORM_OS_IPAD or platform == cc.PLATFORM_OS_MAC then
			cc.FileUtils:getInstance():addSearchPath("temp");
		elseif platform == cc.PLATFORM_OS_ANDROID then
			cc.FileUtils:getInstance():addSearchPath("res/temp");
		else -- windows
			cc.FileUtils:getInstance():addSearchPath("res/temp");
		end
	end
	...
	local sprite = cc.Sprite:create("bg_8.png");
	...
	sprite = cc.Sprite:create("bg_wolf.png");
	...
	sprite:setAnchorPoint(0.5, 0.5);
	sprite:setPosition(vsize.width/2, vsize.height/4*3);
	sprite:setScale(vsize.width / 640);
	layer:addChild(sprite);
	...
	item = cc.MenuItemImage:create("btn_22.png", "btn_22_s.png");
	item:setAnchorPoint(0.5, 0);
	item:setPosition(vsize.width/2, 100);
